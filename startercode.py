...
def search_breed(breed_id):
    ...
    if not isinstance(parsed, dict):
        return None
    block = parsed.get("data")
    if not isinstance(block, dict):
        return None
    if block.get("id") != breed_id:
        return None
    attrs = block.get("attributes")
    if not isinstance(attrs, dict) or "name" not in attrs:
        return None
    return (parsed, url)
...
def update_cache(breed_ids, cache_file):
    ...
    for breed_id in breed_ids:
        ...
        if url in cache:
            continue
        found = search_breed(breed_id)
        if found is not None:
            parsed, _ = found
            cache[url] = parsed
            successful_new_adds += 1
    create_cache(cache, cache_file)
    percentage = (successful_new_adds / n * 100) if n else 0.0
